    ...
    List.fold ps
      ~f:(fun so_far p ->
        if Float.compare p 0.0 < 0 then
          failwith "probabilities must be strictly positive"
        else so_far +. p)
      ~init:0.0
  in
...
let uniform xs =
  ...
  | len when len > 0 ->
      let p = 1.0 /. Float.of_int len in
      enum xs (List.init len ~f:(fun _ -> p))
...
let ( >>>= ) g f sc =
  ...
  let rec loop s =
    ...
    | Cons ((subst, _), rest) ->
        ...
        let fd =
          List.fold
            (stream_to_lst (call_empty_state fg))
            ~init:[]
            ~f:(fun acc (subst, _) -> subst_to_vals subst @ acc)
        in
...
let join_with f d1 d2 =
  d1 >>>= fun x ->
  d2 >>>= fun y -> certainly (f x y)
