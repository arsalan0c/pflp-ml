...
let palindrome str = match str with
...
let explode s =
  let rec loop i l =
    ...
    else let c = String.get s i in loop (i + 1) l@[c]
  in loop 0 []

let consecutive_b str = match str with
  | Atom (Str s) -> 
      let rec loop l = begin match l with 
      | [] -> false
      | 'b'::'b'::_ -> true
      | _::rest -> loop rest
      end
...
let run () = begin
  printf "random string of length 3\n%s" (stream_print (random_string 3 empty_state));
