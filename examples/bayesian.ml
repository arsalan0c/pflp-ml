...
let sprinkler rain = match rain with
...
let grass_wet sprinkler rain = match sprinkler, rain with
...
let grass_wet_when_rain = (fun g -> match g with Atom (Bool b) -> b | _ -> assert false) $$
  (let r = (Atom (Bool true)) in sprinkler r >>>= fun s -> grass_wet s r) 

(* P(grass_wet and rain) = P(grass wet | rain)P(rain) *)
let grass_wet_and_rain = (fun g -> match g with Atom (Bool b) -> b | _ -> assert false) $$
  (join_with (fun g r -> match g,r with | Atom (Bool g), Atom (Bool r) -> Atom (Bool (g && r)) | _ -> assert false) 
  (let r = (Atom (Bool true)) in sprinkler r >>>= fun s -> grass_wet s r) rain)

(* P(rain | grass wet) = P(grass wet and rain) / P(grass wet) *)
let rain_when_grass_wet = (grass_wet_and_rain empty_state) /. (grass_wet_true empty_state)
...
let grass_wet_and_rain_ = (fun model -> match model with
  | Atom (Lst [Bool b_r; _; Bool b_g]) -> b_r && b_g
  | _ -> assert false
) $$ grass_model

(* conditional probabilities *)
let cond_prob ps dx = (fun model -> match model with
  | Atom (Lst l) -> begin 
    match List.fold2 l ps ~init:true ~f:(fun so_far a b2 -> 
      begin match b2, a with
...
let rain_when_grass_wet_ = (grass_cond_prob [true; false; true] empty_state) /. 
  (grass_cond_prob [false; false; true] empty_state)

let sprinkler_when_grass_wet = (grass_cond_prob [false; true; true] empty_state) /. 
  (grass_cond_prob [false; false; true] empty_state)
  
let%expect_test "grass_model" = begin
